...
router.get("/getNowPlaying", async function (req, res, next) {
  ...
  const url = `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.API_MOVIE_KEY}&language=fr-FR&sort_by=popularity.desc&include_adult=false&include_video=false&page=1&release_date.gte=2022-04-01&release_date.lte=2022-06-15`;
  const response = await request("GET", url);
